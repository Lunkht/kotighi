...
<style>
  ...
  .alert-success {
      ...
      border-radius: 10px; padding: 16px; color: #00f5c4;
      font-family: 'Space Mono', monospace;
  }
  .alert-warning {
      background: rgba(255,165,0,0.08); border: 1px solid rgba(255,165,0,0.3);
      border-radius: 10px; padding: 16px; color: #ffa502;
      font-family: 'Space Mono', monospace;
  }
  .info-banner {
      background: rgba(124,108,255,0.08); border: 1px solid rgba(124,108,255,0.2);
      border-radius: 10px; padding: 12px 16px; color: #9d8fff;
...
# ==============================================================
#  SIDEBAR — NAVIGATION (sans emojis dans les valeurs)
# ==============================================================
...
with st.sidebar:
    ...
    # ⚠️ CORRECTION CLÉ : les labels du radio correspondent EXACTEMENT
    #    aux conditions if/elif plus bas
    page = st.radio("", [
        "Accueil",
        "Cybersécurité",
        "Santé",
    ...
    st.markdown("""
    <div style="font-family:'Space Mono',monospace; font-size:0.7rem;
                color:#444460; text-align:center; margin-top:16px">
...
if page == "Accueil":
    ...
    with col1:
        ...
        <div style="background:#111118; border:1px solid #1e1e2e; border-radius:16px; padding:28px">
            ...
            <div style="margin-top:16px; font-family:'Space Mono',monospace;
                        font-size:0.75rem; color:#444460">
    ...
    with c1: st.metric("Précision cyber", "98%+", "avec NSL-KDD réel")
    with c2: st.metric("Précision santé", "95%+", "sur données réelles")
    with c3: st.metric("Types d'attaques", "5", "DoS, Probe, R2L...")
    with c4: st.metric("Symptômes analysés", "10", "par le module santé")

    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown("""
    <div class="info-banner">
    💡 <strong>Comment utiliser :</strong> Clique sur <strong>Cybersécurité</strong>
    ou <strong>Santé</strong> dans le menu à gauche pour lancer une analyse.
    Le Dashboard montre les statistiques globales.
    </div>
    """, unsafe_allow_html=True)
    ...
    with col2:
        ...
        if analyser:
            features = pd.DataFrame([{
                'requetes_min': requetes, 'duree': duree, 'octets': octets,
                'ports_scanes': ports, 'taux_erreur': taux_erreur,
            ...
            if prediction == 0:
                ...
                if requetes > 2000:    type_attaque = "DoS / DDoS (Flood)"
                elif ports > 30:       type_attaque = "Probe (Scan de ports)"
                elif taux_erreur > 0.7: type_attaque = "Brute Force (R2L)"
                else:                  type_attaque = "Activité suspecte"
                st.markdown(f"""
    ...
    with col2:
        ...
        if analyser_sante:
            ...
            if nb_symptomes == 0:
                ...
                features = pd.DataFrame([{
                    'fievre': int(fievre), 'toux': int(toux), 'fatigue': int(fatigue),
                    'maux_tete': int(maux_tete), 'douleur_gorge': int(douleur_gorge),
                    'nausees': int(nausees), 'douleur_thorax': int(douleur_thorax),
                    'essoufflement': int(essoufflement), 'diarrhee': int(diarrhee),
                    'frissons': int(frissons),
                }])
                ...
                fig = px.bar(df_proba, x='Probabilité', y='Diagnostic', orientation='h',
                             color='Probabilité',
                             color_continuous_scale=['#1e1e2e', '#7c6cff', '#ff6b6b'])
                fig.update_layout(
                    paper_bgcolor='#111118', plot_bgcolor='#111118', height=300,
